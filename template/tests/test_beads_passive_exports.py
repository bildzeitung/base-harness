"""The canonical beads passive-export list, and the chain that reaches it (proj-do3q).

`scripts/beads-passive-exports.txt` is the single copy of the two relpaths that the guards
must treat as "by invariant never real work" (`import.auto: false`, proj-6ra). Its consumers
...
restore (proj-2nw5), and `scripts/land-replay.sh`'s two dirty-tree reformat-detect checks
(proj-3cda). Register a new bash consumer in the `BASH_CONSUMERS` tuple below -- every
...
as of proj-y3dw: `flock(1)` replaced the mkdir reclaim gate, retiring the
`LAND_LOCK_TEST_STALL_SECONDS` hook that scan existed to police.)
...
# consumer is a single edit here, not one per loop (proj-3cda's review: the module had
...
# those consumers (proj-xlcm). It is not a member of BASH_CONSUMERS -- it never names the
...
def test_no_consumer_keeps_a_literal_copy_of_the_relpaths() -> None:
    ...
    proj-do3q removed, and would be invisible -- the re-inlined copy would agree with the
...
def test_no_consumer_hardcodes_the_broad_beads_pathspec() -> None:
    """proj-3cda: land-replay.sh used to hardcode a literal ':!.beads' git pathspec
...
def _lib_sourcers() -> list[Path]:
    ...
    Same reasoning as `tests/test_gate_lib.py`'s `_sources_gate_lib` (proj-pcee): a
...
def test_every_sourcer_guards_the_source_line(script: Path) -> None:
    """proj-bss5's fail-closed convention, applied to this library (proj-xlcm).
