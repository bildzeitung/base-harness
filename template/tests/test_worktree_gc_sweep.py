...
cases below assert the *refusals* as hard as the reclaims:
  ...
  * running from a worktree instead of the primary checkout refuses outright
...
def _add_wt(repo: Path, name: str, branch: str, start: str = "main") -> Path:
    _git(repo, "worktree", "add", "-q", "-b", branch, f".claude/worktrees/{name}", start)
    return repo / ".claude" / "worktrees" / name


def _sweep(repo: Path, cwd: Path | None = None) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ["bash", str(repo / "scripts" / "worktree-gc-sweep.sh"), "--base-ref", "main"],
...
def test_everything_skipped_is_distinguishable_from_idle(tmp_path: Path) -> None:
    ...
    assert "reclaimed 0 of 1" in out, "a sweep that reclaimed nothing must not read as idle"
...
def test_bare_ref_backstop_keeps_a_land_ref_whose_remote_still_exists(tmp_path: Path) -> None:
    ...
    assert "land/t1--agent-xyz" in branches, "suffixed ref deleted despite its remote existing"


def test_bare_ref_backstop_deletes_a_land_ref_whose_remote_is_gone(tmp_path: Path) -> None:
    origin = tmp_path / "o"
    origin.mkdir()
    _git(origin, "init", "-q", "--bare", "-b", "main", ".")
    repo = _repo(tmp_path)
