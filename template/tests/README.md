...
| **Cross-file invariants** | `test_validate_sha40_call_sites`, `test_no_hand_derived_skill_md_path`, `test_sweep_pipeline_label_roster_gate`, `test_land_loops_shared_idioms` (the two merge loops' hand-copied idioms stay byte-identical), `test_beads_passive_exports` (every consumer still reads the canonical passive-export list) | they pin that two files agree — the exact thing that drifts silently |
