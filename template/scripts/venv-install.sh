...
# copy of the locked venv install sequence. Before this, the two
...
#      like the step an earlier fix removed: it is cosmetic, not dead
...
#      docs/stack.md#dependency-locking-an earlier fix.
...
#      build-time copy instead of the editable one.
#
#      Do NOT re-add a separate `uv pip install -e. --no-deps` step before
#      this one: measured as a pure no-op -- same package set,
...
#      way. Full record: docs/stack.md#dependency-locking-an earlier fix.
#
...
# scripts/update-deps.sh: a bare-statement chain reached the
