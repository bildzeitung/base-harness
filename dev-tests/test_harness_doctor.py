"""Tests for scripts/harness-doctor.sh.
...
mocked subprocess -- sabotage-provable: reverting any one check in the
script would turn the corresponding test here red.
"""
...
# Beads is deliberately NOT assumed on the test runner, so a `bd` stub goes
...
_STUB_BIN = Path(tempfile.mkdtemp(prefix="harness-doctor-stub-bin-"))
...
def _build_healthy_repo(tmp_path: Path) -> Path:
    ...
    (beads_dir / "config.yaml").write_text('issue-prefix: "proj"\nimport.auto: false\n')

    tests_dir = repo / "tests" / "harness"
...
def test_missing_prerequisite_fails(tmp_path: Path) -> None:
    """The stub must not make the prerequisite check unfalsifiable.
...
def test_import_auto_invariant(
    ...
    repo = _build_healthy_repo(tmp_path)
    (repo / ".beads" / "config.yaml").write_text(f'issue-prefix: "proj"\n{config}')
    result = _run(repo)
...
def test_no_gate_tests_warns_not_fails(tmp_path: Path) -> None:
    repo = _build_healthy_repo(tmp_path)
    (repo / "tests" / "harness" / "test_stub.py").unlink()
    result = _run(repo)
    assert result.returncode == 0, result.stdout + result.stderr
