...
``.claude/agents/*.md``) Path expression may survive outside the shipped
tests/harness/conftest.py (proj-1el8).

DEV-ONLY (lives in the export's dev-tests/, never installed): it gates the
harness test suite's own DRY-ness -- both the shipped tests/harness/ modules
and this directory -- which protects the suite as it grows here, not the
harness a served project runs. Path references below that say
``tests/conftest.py`` describe the layout before the suite moved under
tests/harness/; the mechanism is unchanged.

proj-b8jc consolidated five hand-derived copies of the sweep SKILL.md path
(``REPO_ROOT / ".claude" / "skills" / "sweep" / "SKILL.md"``) into
tests/conftest.py's ``SWEEP_SKILL``, next to ``LAND_SKILL`` (itself
consolidated from four copies by proj-va47). The duplication had already
RE-FORKED once during proj-b8jc's own lifetime: tests/test_sweep_source_query_failure.py
landed on main (236a17c) carrying a fresh hand-derived copy AFTER the first
consolidation attempt (proj-vqlx) was written -- the ticket predicted this
...
Modeled on tests/test_no_private_fence_state_machine.py's shape (proj-k5qb):
...
all nine consolidated copies (proj-va47, proj-b8jc) were test modules pinning
...
REPO_ROOT = EXPORT_ROOT / "template"  # the harness under test
#: Both places a harness test module can live: the shipped suite and this
#: dev-only one. Each has a conftest.py, and the shipped one is the single
#: sanctioned home for the SKILL.md/agent-md constants (dev-tests/conftest.py
#: only re-exports it).
SCAN_DIRS = (REPO_ROOT / "tests" / "harness", EXPORT_ROOT / "dev-tests")
CONFTEST_PATH = REPO_ROOT / "tests" / "harness" / "conftest.py"
EXEMPT = {CONFTEST_PATH, EXPORT_ROOT / "dev-tests" / "conftest.py"}
...
def test_no_hand_derived_skill_md_path_outside_conftest() -> None:
    """THE GATE: every module under SCAN_DIRS, except the conftests (the shipped one is
    ...
    for path in _scan_paths():
        ...
        for lineno, segments in hand_derived_findings(tree):
            ...
            if f"{suggestion} =" in conftest_text:
                howto = f"import {suggestion} from tests/harness/conftest.py instead of re-deriving it"
            else:
                howto = (
                    f"tests/harness/conftest.py has no {suggestion} yet -- add one there "
                    "(following LAND_SKILL/SWEEP_SKILL's pattern) and import it"
                )
            offenders.append(f"{path.relative_to(EXPORT_ROOT)}:{lineno} -- {howto}")
    assert not offenders, (
        "hand-derived SKILL.md/agent-md Path expression(s) found outside "
        f"tests/harness/conftest.py: {offenders} (proj-1el8, proj-b8jc, proj-va47)"
    )
...
def test_gate_catches_the_pre_sweep_skill_shape() -> None:
    """SABOTAGE PROOF (proj-1el8 AC3): the exact hand-derived shape that
    lived, verbatim, in tests/test_sweep_digest_id.py before proj-b8jc
...
"""
    ...
    assert findings, (
        "gate failed to catch the pre-proj-b8jc hand-derived SWEEP_SKILL shape"
    )
...
def test_gate_catches_the_land_skill_shape() -> None:
    ...
    proj-va47): ``_CHECKOUT_ROOT / ".claude" / "skills" / "land" /
    "SKILL.md"``."""
    src = """
...
"""
    tree = ast.parse(src)
    findings = hand_derived_findings(tree)
    assert findings, "gate failed to catch the pre-proj-va47 LAND_SKILL shape"
