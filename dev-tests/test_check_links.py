"""Tests for scripts/check_links.py, the markdown link/anchor gate.
...
REPO_ROOT = (
    Path(__file__).resolve().parent.parent / "template"
...
#: `docs/<path>.md` (anchored or not) reference -- the real-repo
...
#: the ones with an explicit anchor -- an anchor-LESS bare
...
class TestGithubSlug:
    ...
    def test_inline_code_span_contributes_bare_text(self):
        assert github_slug("Fix `proj-o7pf` now") == "fix-proj-o7pf-now"
    ...
    def test_punctuation_is_deleted_not_replaced_with_space(self):
        ...
        assert github_slug("not built (proj-o7pf)") == "not-built-proj-o7pf"
...
class TestCheck:
    ...
    def test_files_outside_docs_and_dot_claude_are_valid_link_targets(self, tmp_path):
        ...
        gets the full markdown-link walk in its own right --
...
class TestScanScopeWidenedRepoWide:
    """The full bracketed-link walk is no longer bounded to
    ...
    def test_broken_bracketed_link_in_nested_readme_is_reported(self, tmp_path):
        """The concrete case that motivated the widening: a bracketed link
...
class TestBareDocAnchorRefs:
    """A bare-text `docs/<path>.md#<anchor>` reference -- no
    ...
    def test_anchor_less_bare_reference_to_existing_file_passes(self, tmp_path):
        """A bare `docs/<page>.md` reference with NO `#anchor` --
        the exact shape a `help=` docstring footnote writes -- must resolve when the file
    ...
    def test_anchor_less_bare_reference_to_missing_file_is_reported(self, tmp_path):
        """The core acceptance case: an anchor-less citation to a
    ...
    def test_excluded_dir_files_are_excluded_from_the_bare_pass(self, tmp_path):
        ...
        bare pass (a markdown file OUTSIDE it is claimed by both -- see TestScanScopeWidenedRepoWide). Asserted on the pass
    ...
    def test_fence_rule_is_markdown_only_so_a_py_docstring_fence_hides_nothing(
        ...
        still cite a doc anchor after it -- noxfile.py can carry real docstring
        citations. Skipping fenced lines
...
@pytest.mark.skip(
    reason="the harness template deliberately cites docs/ pages the HOST project "
    "authors (docs/decisions.md, docs/configuration.md, ...), so a fresh install "
    "fails this self-test until those pages exist. Delete this skip once your "
    "project's docs/ is in place -- the gate itself (nox/CI) is what enforces it."
)
def test_real_repo_passes_the_gate():
    """The acceptance criterion, for both passes at once: every relative
    markdown link in every tracked *.md file (repo-wide, not just
