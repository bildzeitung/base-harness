...
applies to a ticket (proj-mm73 item 1; discovered while technically reviewing proj-ppki).
...
The repo has already answered this exact class of problem twice: `proj-jhry` deleted a gate roster
from `agents-workflow.md` as the staleness anti-pattern, and `proj-200t` added
...
does anything" shape `proj-ppki`'s design deliberately keeps narrow (see §2b's own "deliberately not
...
def test_every_pipeline_label_is_covered_by_sweep_2b_or_documented_epic_only() -> None:
    """The actual gate (proj-mm73 item 1). Every label this repo applies to a TICKET via
